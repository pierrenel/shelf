...
function Gallery({ items, open }: {
    ...
    <div className="card-caption"><h2>{item.title || item.domain}</h2><p>{item.favicon_path && <img src={item.favicon_path} alt="" width="14" height="14"/>}{item.domain}{item.reading_minutes && <span className="reading-badge">· {item.reading_minutes} min · {item.is_read ? 'Read' : 'Unread'}</span>}<Icon name="external" width="12" height="12"/></p>{!!item.tags.length && <div className="card-tags">{item.tags.slice(0, 3).map(name => <span key={name}>{name}</span>)}</div>}</div>
  </a>)}</div>;
}
function Detail({ id, revision, close, notify, previous, next }: {
    ...
    const [error, setError] = useState('');
    const [view, setView] = useState<'watch' | 'read' | 'full' | 'viewport'>('full');
    ...
    useEffect(() => { let active = true; api<Item>(`/api/items/${id}`).then(data => { if (active) {
        setItem(data);
        if (!chosenView.current && (youtubeVideo(data.url) || data.article)) { setView(youtubeVideo(data.url) ? 'watch' : 'read'); chosenView.current = true; }
    ...
    const action = async (kind: 'recapture' | 'delete') => { setBusy(true); setError(''); try {
        if (kind === 'delete') {
            ...
            notify('Refreshing the screenshot and reading copy.');
        }
    }
    catch (e) {
        setError((e as Error).message);
    ...
    {item.article ? <section className="reading-status"><h2>{item.reading_minutes} min read · {item.is_read ? 'Read' : 'Unread'}</h2><button className="secondary" disabled={busy} onClick={() => void update({ is_read: !item.is_read })}>{item.is_read ? 'Mark unread' : 'Mark as read'}</button></section> : !video && item.status === 'done' && <p className="reading-unavailable">No reading copy saved. Recapture an older bookmark to try extracting its article.</p>}
