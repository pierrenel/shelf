...
function Detail({ id, revision, close, notify, previous, next }: {
    ...
    const [error, setError] = useState('');
    const [view, setView] = useState<'read' | 'full' | 'viewport'>('full');
    ...
    useEffect(() => { let active = true; api<Item>(`/api/items/${id}`).then(data => { if (active) {
        setItem(data);
        if (!chosenView.current && data.article) { setView('read'); chosenView.current = true; }
    ...
    {item.article ? <section className="reading-status"><h2>{item.reading_minutes} min read · {item.is_read ? 'Read' : 'Unread'}</h2><button className="secondary" disabled={busy} onClick={() => void update({ is_read: !item.is_read })}>{item.is_read ? 'Mark unread' : 'Mark as read'}</button></section> : item.status === 'done' && <p className="reading-unavailable">No reading copy saved. Recapture an older bookmark to try extracting its article.</p>}
